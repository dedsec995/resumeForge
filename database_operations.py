...
class DatabaseOperations:
    ...
    def createSession(self, userId, jobDescription):
        ...
        try:
            ...
            sessionRef = self.db.collection('users').document(userId).collection('sessions').document(sessionId)
    ...
    def getAllSessions(self, userId, limit=10):
        ...
        try:
            sessionsRef = self.db.collection('users').document(userId).collection('sessions')
            query = sessionsRef.order_by('timestamp', direction=firestore.Query.DESCENDING).limit(limit)
